...
def LoginClient():
   ...
   def joinGame(ip):
      ...
      try:
         ...
         print("In Client Login (joinGame)")
         print("Failed somewhere  " + str(e))
   ...
   def display_servers(x_panel_position, y_panel_position, y_offset):
      ...
      for server in l_servers:
         ...
         LOGIN_TOP_SURFACE.blit(SERVER_FONT.render("("+str(server[0])+"/7)", True, (0,0,0)), (x_panel_position + 2500, y_panel_position + 25 + y_offset))
   ...
   OK_UNLIT = pygame.image.load(IMAGE_FILE_PATH + "OKUnlit.png").convert_alpha()
