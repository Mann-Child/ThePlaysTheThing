      ...
      for event in pygame.event.get():
         ...
         if event.type == MOUSEBUTTONDOWN:
            ...
            if event.button == 1:
               ...
               # clicked play
               if x_play_button <= x_mouse_position_main <= x_play_button + 200 and y_play_button <= y_mouse_position_main <= y_play_button + 200 and servername != "":# and len(clients) > 0: #Removed to start game with one person
                  start_game()
                  server_socket.close()
                  serversocket.close()
                  for client in clients:
